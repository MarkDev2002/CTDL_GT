...
void AddNewTail(LIST &l, int x)
{
    // chuyển data -> node
    NODE *p = GetNode(x); // p giữ địa chỉ data : x
    if (l.pHead == NULL)  // TH1 : DS rỗng
    {
        l.pHead = l.pTail = p; // thay đổi
    }
    else // TH2:có n pt
    {
        l.pTail->pNext = p; // liên kết p với pHead
        l.pTail = p;        //đặt head ở p(vt đầu mới)
    }
}
...
int main()
{
    ...
    OutPut(l);
    XuLy(l);
    cout << "Danh sach sau khi them node moi vao cuoi danh sach : ";
    OutPut(l);
    GiaiPhong(l);
    cout << endl;
    return 0;
